...
- [`bot/src/services/music.py`](../bot/src/services/music.py) — thin Wavelink wrapper: `connect_node(client, host, port, password)` (idempotent), `play(voice_channel, query, requester_id=None)`, plus queue helpers. The cog calls these and adds no Discord-specific glue beyond what Wavelink already provides.
- [`bot/src/services/voice_recovery.py`](../bot/src/services/voice_recovery.py) — voice-gateway-wedge detection + recovery. Holds the pure `should_recover` decision function (17 unit tests), the `VoiceHealthSnapshot`/`GuildRecoveryState` types, the Lavalink probe HTTP fetchers, and the `reconnect_player_at_position` dance. Used by the music cog's heartbeat task + `on_wavelink_websocket_closed` handler.
- [`bot/src/cogs/embeds.py`](../bot/src/cogs/embeds.py) — pure embed renderers shared by `cogs/music.py` and `cogs/valheim.py`. Functions take service-layer dataclasses (`TrackInfo`, `PlayResult`, `InstanceState`, `LiveStatus`) and return `discord.Embed`. No I/O.
