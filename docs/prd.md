...
| `/music clear` | Clear upcoming queue (keep current track playing) |
...
| `/music pause` / `/music resume` | Toggle playback. |
| `/music clear` | Clear the upcoming queue; keep the current track playing + stay in voice. |
| `/music stop` | Stop playback, clear the queue, leave voice. |
| `/music queue` | Show the next 10 queued tracks + currently playing; header reports the true total (e.g. "Up next (10 of 847)"). |
...
| 2026-05-25 | **YouTube OAuth via GSM-backed refresh token (anti-bot workaround)** | YouTube anti-bot rollout took out the entire youtube-source plugin's unauthenticated client set: WEB / WEB_EMBEDDED_PLAYER / ANDROID_VR all returning `This video requires login` or signature-cipher regex failures (`Must find sig function from script`). Plugin maintainers shipped no fix (latest commit on `lavalink-devs/youtube-source` is from 2026-05-03; no RC, no snapshot diverges from 1.18.1). The plugin's built-in OAuth support is the documented escape hatch: authenticate as a real Google account (burner) and YouTube can't anti-bot us. Built the proper automated wiring rather than the quick-hack `application.yml`-with-hardcoded-token: new GSM secret `lavalink-youtube-oauth-token` (managed by `gcp-bot-vm/secret.tf`), bot-vm SA gets `roles/secretmanager.secretAccessor` on it, `server/lavalink/scripts/fetch-secrets.sh` loads the value into `LAVALINK_OAUTH_REFRESH_TOKEN` at every boot, `application.yml` consumes it via Spring's `${var:}` substitution. Bootstrap is one-time interactive: on first boot the plugin enters device-code mode, prints a URL + code to the journal; operator visits URL on burner account, plugin logs the refresh token, operator seeds GSM via `gcloud secrets versions add`. Subsequent boots are silent (refresh token long-lived, plugin auto-exchanges to access tokens). Trade-off: account-termination risk on the burner is documented but acceptable for hobby volume; rotation is "create new burner + redo OAuth dance + re-seed GSM" (5-min procedure). Discord and the bot's other surfaces (Spotify via lavasrc, Bandcamp) are unaffected; this targets ONLY YouTube playback. Also discovered + fixed a refactor-introduced bug: `on_wavelink_track_exception` was using `getattr(payload.exception, "message", ...)` against a TypedDict, falling through to `repr()` which embedded the entire 4000-char Java stack trace in the user-facing message → Discord 50035 rejection → silent failure. Subscript access + `.splitlines()[0]` + 200-char cap, plus a belt-and-suspenders 1900-char truncation in `_announce`. Documented in runbook scenarios 20 (OAuth bootstrap) and the pre-existing 14 (`Couldn't play X, skipping` flow). |
| 2026-05-26 | **Music fully restored: TV(OAuth) client + remote cipher (yt-cipher) + ytmsearch-first mirrors** | OAuth alone (2026-05-25) wasn't enough — the YouTube breakage had TWO independent failure modes and OAuth only fixed anti-bot. (1) **OAuth client**: only the `TV`/`TVHTML5` client supports OAuth per youtube-source's matrix, so it must lead the `clients` list; without it the refresh token is captured but never *used* on a playback attempt. (2) **Signature cipher**: YouTube changed its player JS to an EJS-style format with PO-token challenges; youtube-source's local sig-extraction regex (`Must find sig function from script`) no longer matches, and there's no upstream fix (same error class hit Red-DiscordBot, pytubefix, yt-dlv this week). Worked around with `plugins.youtube.remoteCipher.url: https://cipher.kikkia.dev/` — kikkia/yt-cipher is a Deno service wrapping yt-dlp/ejs that the plugin calls for deciphering. Using the maintainer's public instance (10 req/s, free); self-host docker-compose is runbook 22 if it flakes. (3) **Spotify mirrors**: lavasrc's `ytsearch:` mirror lookups routed through the still-broken non-music clients → "No mirror found for track"; reordered `providers` to prefer `ytmsearch:` (YouTube Music, which survives anti-bot) with `ytsearch:` as fallback. After all three, YouTube search/URL + Spotify track/album URLs all play. **Known non-fix**: Spotify *editorial* playlists (`37i9...` — Today's Top Hits, Daily Mix, Discover Weekly) 404 from the Spotify Web API since their Nov-2024 third-party restriction; not our bug, no workaround possible. User playlists + albums + tracks work. |
| 2026-07-23 | **youtube-plugin 1.18.1 → pinned `main` snapshot (`c46ab888`)** | OAuth + remote-cipher got music mostly working but left track-specific residual failures on 1.18.1: `TVHTML5: Something went wrong when decoding`, `WEB: No supported audio streams available`, itag/format errors. The youtube-source maintainer had been fixing these on `main` for months (itag-18 playback fix 2026-07-16, client-version updates 2026-07-05, Spring compat 2026-07-16) but never cut a release — maven still lists 1.18.1 as latest. Switched the plugin coordinate to the snapshot maven with the exact commit hash `c46ab888ee12bc4bcb37c8ebcaf180e85ae30368` (pinned, NOT floating `-SNAPSHOT`, so an upstream regression can't silently land). Deploy required removing the cached `youtube-plugin-1.18.1.jar` from `/opt/lavalink/plugins/` so Lavalink downloads the snapshot cleanly. Verified end-to-end via the plugin's `GET /youtube/stream/{videoId}` REST route: the previously-failing "Nobody Rides for Free (2007 Remaster)" returned HTTP 200 + 5.3MB of actual audio (proves format-load + cipher + stream, not just metadata). OAuth + remoteCipher stay configured (complementary; harmless as fallbacks). To bump: check the youtube-source commit log, pick a newer hash, update the coordinate, delete the old plugin jar, restart lavalink. To revert: 1.18.1 + releases repo + `snapshot: false`. |
| 2026-05-26 | **`/music clear` + queue cap 100 → 1000** | Two user-requested features. (1) `/music clear` empties the upcoming queue but keeps the current track playing and stays in voice — fills the gap between `/music skip` (advance one) and `/music stop` (stop + clear + disconnect). New `clear_queue(guild) -> int` service fn (also clears `auto_queue` defensively though partial-autoplay keeps it empty); cog command is `@requires_channel` + `@requires_guild`. (2) `PLAYLIST_TRACK_CAP` 100 → 1000, kept in sync with the three Lavalink limits (`youtubePlaylistLoadLimit`, `lavasrc.spotify.{playlistLoadLimit,albumLoadLimit}`). Memory is a non-issue (~2-3KB/Playable metadata, mirror resolution stays lazy at play-time, so a 1000-track load is metadata paging not 1000 searches); Spotify pages by 100 so a full load is ≤10 API calls (~3s, within the deferred-interaction window). Bonus: `/music queue` now shows "Up next (10 of N)" via a new `queue_length()` so a 1000-track queue isn't misrepresented as the 10 listed. 5 new cog tests + updated truncation test (realistic 1500→1000). |
